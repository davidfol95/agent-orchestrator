...
// Mock quality-gates so tests can control gate outcomes without spawning processes
vi.mock("../quality-gates.js", () => ({
  runAllQualityGates: vi.fn(),
}));
import { runAllQualityGates } from "../quality-gates.js";

// Mock node:child_process so the security-scan handler's git rev-parse HEAD call
// succeeds without requiring a real git workspace
...
describe("auto-merge ordering", () => {
  /** Build a PluginRegistry whose "scm" slot returns the given SCM. */
  ...
  it("does NOT call enableAutoMerge when session first transitions to pr_open (no quality gates configured)", async () => {
    ...
    const lm = createLifecycleManager({
      ...
      registry: makeRegistryWithSCM(mockSCM),
      sessionManager: mockSessionManager,
    });
  ...
  it("does not call enableAutoMerge when SCM plugin does not support it", async () => {
    ...
    const lm = createLifecycleManager({
      config,
      registry: makeRegistryWithSCM(mockSCMWithout),
      sessionManager: mockSessionManager,
    });
  ...
  it("enableAutoMerge failure is swallowed and does not prevent check from resolving", async () => {
    // Deferred reject avoids Vitest's unhandled-rejection guard during check()
    ...
    // check() resolves before enableAutoMerge settles (fire-and-forget)
    await expect(lm.check("app-1")).resolves.toBeUndefined();
    expect(lm.getStates().get("app-1")).toBe("pr_open");

    // Trigger the rejection — lifecycle manager's .catch() should swallow it
    triggerReject(new Error("GitHub API rate limited"));
    await new Promise<void>((resolve) => setTimeout(resolve, 0));
  });
});

describe("quality gates + auto-merge race condition", () => {
  /** Build an SCM that puts the session in pr_open state. */
  function makePrOpenSCMForGates(overrides: Partial<SCM> = {}): SCM {
    return {
      name: "mock-scm",
      detectPR: vi.fn(),
      getPRState: vi.fn().mockResolvedValue("open"),
  ...
  function makeConfigWithQualityGates(): OrchestratorConfig {
    return {
      ...config,
      projects: {
        "my-app": {
      ...
      // "security-scan" is the reaction key for the pr.created event
      reactions: {
        "security-scan": {
          auto: true,
          action: "security-scan" as const,
        },
      },
    };
  }

  function makeRegistryWithSCMForGates(scm: SCM): PluginRegistry {
    return {
      ...mockRegistry,
      get: vi.fn().mockImplementation((slot: string) => {
        if (slot === "runtime") return mockRuntime;
  ...
  it("calls enableAutoMerge exactly once (via gates, not pr_open handler) when quality gates pass", async () => {
    const enableAutoMerge = vi.fn().mockResolvedValue(undefined);
    ...
    // enableAutoMerge should be called exactly once — from the quality gates callback
